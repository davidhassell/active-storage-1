...
if __name__ == "__main__":
    ...
    # inside the dask library. It can be moved to the client code that
    # calls dask. This has not been done in this client code for
    # clarity of demonstration of the client-side approach.
    # ----------------------------------------------------------------
    try:
    ...
    # ----------------------------------------------------------------
    # Lazy read of the netCDF file: active and normal versions
    #
    # 'f' support active storage for some operations
    # 'x' does not support active storage for any operations
    # ----------------------------------------------------------------
    f = NetCDFArray(
        filename="file.nc", ncvar="q", shape=(5, 8), dtype=np.dtype(float)
    )
    ...
    # ----------------------------------------------------------------
    # Compare some active operations with their normal counterparts
    # ----------------------------------------------------------------
    g = da.max(df)
    y = da.max(dx)
